...
OBJ newYbBuiltinFunction(char *name, ybFctPtr implementation) {
	...
	obj =
			(struct ybBuiltinFunction *) (malloc(
					sizeof(struct ybBuiltinFunction)));
...
//------------------------
OBJ newYbUserDefinedFunction(char *name, OBJ env, OBJ parameterList,
		OBJ bodyList, int numDefs) {
...
OBJ newYbEnvironment(int envSize, OBJ parentEnv) {
	ybEnvironment *obj;
	obj = (ybEnvironment *) (malloc(
			sizeof(ybEnvironment) + sizeof(keyValuePair) * envSize));
